...
let CleanPMs = {
    ...
    build_pm_types_tabs: (parent, pms)=> {
        ...
        let tabs = pm_types.map(type => {
            return {
                "name": type,
                "cont": (content_div)=>{
                    CleanPMs.summarize_pm_type(content_div, type, pms);
                },
    ...
    draw: (pms)=>{
        let parent = document.getElementById("Clean PMs");
        let filter_pms = pms["clean_pms"];
        /** reset content **/
        parent.innerHTML = '';

        /** draw types menu **/
        CleanPMs.build_pm_types_tabs(parent, filter_pms);
    }
};