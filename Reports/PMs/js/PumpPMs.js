...
let PumpPMs = {
    ...
    build_pm_types_tabs: (parent, pms)=> {
        ...
        let tabs = pm_types.map(type => {
            return {
                "name": type,
                "cont": (content_div)=>{
                    PumpPMs.summarize_pm_type(content_div, type, pms);
                },
    ...
    draw: (pms)=>{
        let parent = document.getElementById("Pump PMs");
        let filter_pms = pms["pumps_pms"];
        /** reset content **/
        parent.innerHTML = '';

        /** draw types menu **/
        PumpPMs.build_pm_types_tabs(parent, filter_pms);
    }
};