...
let SuperCleanPMs = {
    ...
    build_pm_types_tabs: (parent, pms)=> {
        ...
        let tabs = pm_types.map(type => {
            return {
                "name": type,
                "cont": (content_div)=>{
                    SuperCleanPMs.summarize_pm_type(content_div, type, pms);
                },
    ...
    draw: (pms)=>{
        let parent = document.getElementById("Super Clean PMs");
        let filter_pms = pms["super_clean_pms"];
        /** reset content **/
        parent.innerHTML = '';

        /** draw types menu **/
        SuperCleanPMs.build_pm_types_tabs(parent, filter_pms);
    }
};